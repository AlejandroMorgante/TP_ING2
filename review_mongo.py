...
#Caso de Uso 3 - Propiedades agregadas recientemente
...
#Caso de Uso 4 - Zonas más frecuentes en reservas para un país específico
def areas_demandadas_por_pais(pais):
    client = MongoClient("mongodb://localhost:27017")
    db = client["test"]
    
    print("\nÁreas más demandadas por país:")

    pipeline = [
        {
            "$match": {
                "$or": [
                    { "hotel.ubicacion.pais": pais },
                    { "paquete.hotel.ubicacion.pais": pais }
                ]
            }
        },
        {
            "$project": {
                "zona": { "$ifNull": ["$paquete.hotel.zona", "$hotel.zona"] }
            }
        },
        {
            "$group": {
    ...
    for doc in resultados:
        zona = doc["_id"] or "No especificada"
        total = doc["total"]
        print(f"- Zona: {zona} | Total demandas: {total}")
...
def consulta_alojamientos_baratos_o_centrico():
    ...
    for doc in resultados:
        ...
        zona = doc.get("zonaCentrica", False)
        print(f"- Tipo: {tipo} | Precio: {precio} | Zona: {'Centro' if zona else 'No Centrica'}")
